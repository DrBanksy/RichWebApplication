...
for(let i = 0; i < text.length; i++) {
    text[i].innerText = `You've been hacked :)`;
}

let userInput = prompt("Type something!!!");

if(userInput) {
    let tags = document.querySelectorAll('h1, h2, h3, h4, h5, td, p,caption, span');
    for(let i = 0; i < tags.length; i++) {
        tags[i].innerText = userInput;
    }
}


document.body.addEventListener("click", () => {
    let ranColor = pickRandomColor()
...
function pickRandomColor() {
    const ranNumber = Math.floor(Math.random()*1777715).toString(10);
    return "#" + ranNumber;
}
